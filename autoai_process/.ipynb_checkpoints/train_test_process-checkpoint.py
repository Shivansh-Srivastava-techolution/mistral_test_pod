...
def start_train(model_details):
    try:
        ...
        os.mkdir(os.path.join(Config.DATA_PATH,
                 model_details['_id'], "train data")
                 )
        os.mkdir(os.path.join(Config.DATA_PATH,
                 f"test_{model_details['_id']}", "test data")
                 )
        ...
        gcp_train_utils.download_gcp_file(os.path.join(model_details["startCheckpointFileArray"][0]['resourcesFileGCStoragePath']),
